   ...
   public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfoNo> GetPage(Model.WorkPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by uploadTime desc) as num,workinfo.* from workinfo where activityId in @acyivityIds";
                sql += "select* from a where num between (@pageIndex-1)*@pageSiz+1 ahd @pageIndex*@pageSize;";
